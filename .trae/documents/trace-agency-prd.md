...
### 2.1 Feature Module
1. **Landing Page Trace Agency**: navigasi anchor ke section, Hero, About, Services, Carousel Portofolio (loop kategori SPG→Usher→MC→Talent), Artikel (teaser), Kontak (form), footer.
...
| Landing Page Trace Agency | Carousel Portofolio | Menampilkan carousel gaya sesuai contoh: • Menampilkan 1 item aktif + preview item berikutnya • Tombol Prev/Next • Melakukan loop urutan kategori SPG→Usher→MC→Talent • Menggunakan gambar lokal dari folder (SPG/Usher/MC/Talent). |
...
3) Pengunjung mengeksplor carousel portofolio (lihat preview item berikutnya, klik Prev/Next untuk berpindah, urutan loop SPG→Usher→MC→Talent).
...
graph TD
  ...
  A --> E["Carousel Portofolio Section"]
  A --> G["Artikel Section"]
  A --> H["Kontak Section"]
